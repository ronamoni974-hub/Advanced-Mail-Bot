...
system_data = {'active_promos': {}, 'bot_active': True, 'admins': [DEVELOPER_ID]} 

# API Data Structure for Dual Servers
api_data = {
    'mailtd_tokens': [], 
    'tmailor_tokens': [], 
...
def load_all_data_from_firebase():
    ...
    try:
        ...
        if ban_doc.exists: 
            ...
            # Admin Protection from Firebase DB
...
# --- Load Balancing & Premium API Engine ---
def restore_apis():
...
def get_active_client(server_type='mailtd', exclude_tokens=None):
    ...
    if not valid_tokens: 
        raise Exception(f"No API Tokens found for {server_type.capitalize()}! Please add API from Admin Panel.")
    ...
    raise Exception(f"All {server_type.capitalize()} APIs Exhausted!")

def create_mail_with_server(chat_id, clean_name=None):
    preferred = user_data[chat_id].get('server_pref', 'mailtd')
    token_key = f"{preferred}_tokens"
    
    if not api_data.get(token_key, []):
        raise Exception(f"⚠️ {preferred.capitalize()} সার্ভারে কোনো API সেটআপ করা নেই! দয়া করে অ্যাডমিন প্যানেল থেকে API Token অ্যাড করুন।")
        
    failed_tokens = set()
    for _ in range(len(api_data.get(token_key, []))):
        try:
            client, token = get_active_client(preferred, exclude_tokens=failed_tokens)
            if preferred == 'mailtd':
                ...
                return account.id, account.address, token, preferred
            elif preferred == 'tmailor':
                ...
                return "tmailor_acc", email_addr, token, preferred
        except Exception as e:
    ...
    raise Exception(f"All {preferred.capitalize()} APIs Failed!")
...
def generate_mail_layout(email_address, srv_type):
    server_name = "Premium Mail.td API" if srv_type == 'mailtd' else "Premium Tmailor.com API"
...
def send_welcome(message):
    ...
    if not system_data.get('bot_active', True) and not is_admin(message.chat.id):
...
def handle_text(message):
    ...
    if text == "✨ Generate Premium Mail":
        ...
        try:
            ...
            elif srv_type == 'tmailor': api_data['usage'][used_token] = api_data['usage'].get(used_token, 0) + 1
...
def broadcast_promo(message, promo_message):
    bot.clear_step_handler_by_chat_id(message.chat.id) # Prevents double triggering
...
def handle_callback(call):
    ...
    if call.data.startswith('cp_'):
        bot.answer_callback_query(call.id) # Silent answer to avoid popup
...
if __name__ == "__main__":
    ...
    print("🚀 Pro Mail Bot (Unified Architecture + Admin Core) is Live...")
    while True:
